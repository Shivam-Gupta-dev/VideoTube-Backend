...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if ([fullName, email, username, password].some((field) => field ? field.trim() === '' : true)) {
...
const loginUser = asyncHandler(async (req, res) => {
    // collect user crediantials
    // verify if user is registered or not 
    // if not registered redirect to signup
    // if user exist but password doesn't correct redirect login and give option to forgot password
    // if user is registered then login
    // generate access token and refresh token
    // send response user successfully logged in with token
    ...
    // try user.refreshtoken = ''
    // user.password = ''

    const options = {
        // by using this cokkie will modified by server only not by frontend/user
        httpOnly: true,
        secure: true,
    }
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        return res
            ...
            .cookie("refreshToken", newRefreshTokenefreshToken)
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
    ...
    if (!coverImageLocalPath) throw new ApiError(401, "Avatar file is missing")

    const coverImage = await uploadOnCloudinary(coverImageLocalPath)

    if (!coverImageLocalPath.url) throw new ApiError(500, "Error while uploading on avatar")
